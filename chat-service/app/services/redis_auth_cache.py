...
class RedisTokenCache:
    ...
    async def clear_all(self) -> int:
        ...
        WARNING: This affects all services using the cache.
        ...
        try:
            ...
            logger.error(f"Redis error during clear_all: {e}")
            return 0
        except Exception as e:
            logger.error(f"Unexpected error during clear_all: {e}")
            return 0
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        try:
            # Get metric counters
            hits = int(self.redis_client.get(f"{self.key_prefix}metrics:hits") or 0)
            misses = int(self.redis_client.get(f"{self.key_prefix}metrics:misses") or 0)
            sets = int(self.redis_client.get(f"{self.key_prefix}metrics:sets") or 0)
            errors = int(self.redis_client.get(f"{self.key_prefix}metrics:errors") or 0)
            invalidations = int(self.redis_client.get(f"{self.key_prefix}metrics:invalidations") or 0)

            total_requests = hits + misses
            hit_rate = (hits / total_requests * 100) if total_requests > 0 else 0

            # Count current cached tokens
            pattern = f"{self.key_prefix}*"
            # Exclude metric keys
            all_keys = self.redis_client.keys(pattern)
            cache_size = len([k for k in all_keys if "metrics:" not in k])
            ...
            return {
                ...
                "metrics_window_seconds": self.metrics_window
            }
